...
const ConeCalculator = ({ 
  ...
  const calculateVolume = () => {
    ...
    // Calculate volume in input unit (1/3 × π × r² × h)
    const volumeInInputUnit = (1/3) * Math.PI * Math.pow(r, 2) * h;
    ...
    const totalSurfaceArea = lateralSurfaceArea + baseArea;
    ...
    <div className="bg-surface-light dark:bg-surface-dark-hover rounded-xl p-6 border border-gray-200/10 dark:border-gray-800/10">
      <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="block text-lg font-display font-medium mb-2">Base Radius</label>
            <input
              type="number"
        ...
        {result && (
          <div className="mt-6 space-y-4">
            ...
            <div className="bg-surface-light-hover dark:bg-surface-dark rounded-lg p-4">
              ...
              <p className="text-sm text-content-light-dimmed dark:text-content-dark-dimmed">
                Calculation: (1/3) × π × {result.dimensions.radius}² × {result.dimensions.height} = {result.value.toLocaleString()} {result.unit}
              </p>
            </div>
          </div>
        )}
